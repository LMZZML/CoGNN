...
import util
class Trainer():
    def __init__(self, model, lrate, wdecay, clip, step_size, seq_out_len, bike_size, taxi_size, scaler_bike, scaler_taxi, device, cl=True):
        self.scaler_bike = scaler_bike
        self.scaler_taxi = scaler_taxi
        self.model = model
    ...
    def train(self, input, real_val, idx=None):
        ...
        predict_bike = self.scaler_bike.inverse_transform(output[:,:,:self.bike_size,:])
        predict_taxi = self.scaler_taxi.inverse_transform(output[:,:,-self.taxi_size:,:])
        predict = torch.cat([predict_bike, predict_taxi], 2)
